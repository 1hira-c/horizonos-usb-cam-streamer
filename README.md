...
7. 配信を開始するとフォアグラウンドサービス（`StreamingService`）が常駐し、**別アプリに切り替えても配信が継続**する。停止は常駐通知の「停止」アクション、またはアプリ内の「全停止」ボタンから（全カメラ個別停止でも自動解除）
...
- `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_CONNECTED_DEVICE`（別アプリ使用中の配信継続用）
- `POST_NOTIFICATIONS`（常駐通知の表示用。未許可でも配信自体は継続）
...
- `usb/StreamingService.kt` — `UsbRepository` を所有するフォアグラウンドサービス（`connectedDevice`）。Activity から bind して操作し、別アプリ使用中も配信を継続
